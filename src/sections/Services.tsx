      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service) => (
            ...
            >
              <div className={`w-20 h-20 bg-gradient-to-br ${service.gradient} rounded-2xl flex items-center justify-center text-white mb-6 group-hover:scale-110 transition-transform duration-300 ${isRTL ? 'mr-auto' : 'ml-0'}`}>
                {service.icon}
              </div>
              <h3 className={`text-2xl font-bold text-gray-900 mb-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                {t(`services.${service.key}`)}
              </h3>
              <div className={`w-16 h-1 bg-gradient-to-r ${service.gradient} ${isRTL ? 'mr-auto' : 'ml-0'}`}></div>
            </div>
          ))}
        </div>
      </div>
